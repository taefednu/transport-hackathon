...
# docs/superpowers/ — спеки и планы реализации: рабочие документы команды того же
# класса, что knowledge/. Каталог появился позже этого списка и под шаблон
# *_spec.md не подходит, поэтому назван отдельно.
EXCLUDE_REGEX='^(backend/data/|knowledge/|docs/superpowers/|CLAUDE\.md$|NIGHT_REPORT\.md$|NIGHT_FIXES\.md$|.*_tz\.md$|.*_spec\.md$|frontend-ref\.png$)'
