...
# Сколько ждём модель, прежде чем уйти на запасной путь.
# Было 5 с — при скорости генерации около 200 знаков в секунду этого хватало
# ровно на короткий ответ, а живой вопрос чаще требует длинного: ответы
# обрывались на середине фразы и человеку не отдавались, либо до пересказа
# вообще не доходило. Запасной путь никуда не делся, он просто перестал
# срабатывать там, где модель успела бы ответить по существу.
LLM_TIMEOUT_SEC = float(os.environ.get("QATNOV_LLM_TIMEOUT_SEC", "14"))
...
# Было 340 токенов на 600 знаков — под них ответ приходилось резать до
# телеграфного, и на содержательный вопрос человек получал огрызок либо
# шаблон. Плата за развёрнутый ответ — секунды генерации, и она осознанная.
LLM_MAX_TOKENS_ANSWER = 700
# длина, которую просим соблюдать: около шести секунд генерации
...
# начатый за мгновение до порога: бюджет минус таймаут плюс таймаут, то есть
# сам бюджет. При 30 и таймауте 14 худший случай — 30 секунд.
# Было 8: этого не хватало на выбор инструмента моделью плюс пересказ, и
# на живых вопросах ассистент регулярно отвечал «не осталось бюджета
# времени», хотя посчитано было всё. Ответ по существу дороже трёх секунд.
ASSISTANT_BUDGET_SEC = float(os.environ.get("QATNOV_ASSISTANT_BUDGET_SEC", "30"))
...
#
# `vehicles_short` веса не имеет намеренно. Он сравнивает потребность по плану
# с `n_vehicles` из транзакций, а это уникальные номера бортов, у которых в
# этот час прошла хотя бы одна оплата, — нижняя граница выпуска, а не выпуск.
# Признак срабатывал у 28 маршрутов из 51 и тянул список вверх недоучётом, а не
# нехваткой машин. Считать его продолжаем и показываем в объяснении: это
# наблюдение о данных. В ранжирование оно не идёт.
#
# Сравнивать вместо оплат число рейсов нельзя: `actual_headway = 60 / n_trips`,
# то есть такое сравнение — это `headway_gap`, уже посчитанный с весом 1.0.
ATTENTION_WEIGHTS = {
    "headway_gap": 1.0,
    "duplication": 0.6,
    "route_too_long": 0.5,
...
# нижняя граница интервала. Дело не только в том, что автобусы не ходят чаще
# раза в минуту: расписание строится циклом «пока не кончился рабочий день,
# шаг — интервал», и на 0.001 мин это миллион рейсов по три десятка остановок
# в каждом. Один такой запрос съедает всю память и убивает процесс целиком —
# вместе с ним падает и весь сервер для остальных
MIN_HEADWAY_MIN = 1.0
