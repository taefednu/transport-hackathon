...
На стенде лежат только посчитанные артефакты пайплайна — агрегаты и геометрия.
Выданного датасета там нет: ни строки транзакций, ни одного идентификатора карты.
...
Интерфейс и ядро едут одним образом и одним адресом: собранный фронтенд лежит
рядом с сервером и раздаётся им же, поэтому кросс-доменных запросов нет.

```bash
cd frontend && VITE_API_BASE= npm run build   # фронтенд без адреса ядра — тот же origin
bash backend/scripts/make_deploy.sh /tmp/qatnov-deploy
cd /tmp/qatnov-deploy && railway up
```

`make_deploy.sh` собирает каталог **перечислением**, а не исключением: в выкладку
попадает только то, что читает `app/store.py`, плюс код и собранный фронтенд.
Датасет, дамп OSM, слой Kontur, `.env` и виртуальное окружение туда не попадают,
и скрипт отдельно проверяет это перед выходом. Ключ модели задаётся переменными
